...
def validate_posting_date(doc, method):
    for item in doc.items:
        contract_details = frappe.db.get_list('Contract', {'party_name': doc.supplier, 'custom_project': item.project}, ['end_date','name', 'custom_amount'])
        for i in contract_details:
            if str(i.end_date) < str(doc.posting_date):
                if not doc.custom_reason:
                    ...
                    ''' 
                    frappe.throw(frappe._("Contract has been Expired. Last Date is {0}. If needed kindly access the Contract here - {1}.").format(formatdate(i.end_date), ms))
        ...
        for pb in project_budgeting_amount:
            if doc.custom_total_amount > pb.custom_grand_total:
                ms =f'''
                    ...
                    '''  
                frappe.throw(frappe._("Total Consolidated Amount is greater than Project Budget Grand Total.If you want to proceed then select reason. If needed kindly access the Project Budget here - {0}.").format(ms))
        ...
        for item in self.custom_item_details:
            ...
            for pb in project_budgeting_amount:
                ...
                if pb.amount < item.total_consolidated_budget_account_head_amount:
                    ...
                    '''
                    frappe.throw(frappe._("Total Consolidated Budget Account Head Amount is greater than Budget AccountMapping Amount. If needed kindly access the Project Budget here - {0}.").format(ms))
