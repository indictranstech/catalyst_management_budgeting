...
def validate_posting_date(doc, method):
    for item in doc.expenses:
        contract_details = frappe.db.get_list('Contract', {'party_name': doc.employee, 'custom_project': item.project}, ['end_date','name', 'custom_amount'])
...
def calculate_item_details(self, method):
    ...
    for item in self.expenses:
        ...
        if key in item_details:
            item_details[key] += item.sanctioned_amount
        else:
            item_details[key] = item.sanctioned_amount
    ...
    for key, sanctioned_amount in item_details.items():
        project_budget,budget_account_head = key
        self.append("custom_item_details", {
            "current_invoice_amount": sanctioned_amount,
