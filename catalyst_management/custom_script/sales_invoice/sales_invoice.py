...
def update_chart_of_account(doc, methode):
    for item in doc.items:
        ...
        for i in result:
            item.income_account = i.chart_of_account_head
...
def validate_posting_date(doc, method):
    for item in doc.items:
        contract_details = frappe.db.get_list('Contract', {'party_name': doc.customer, 'custom_project': item.project}, ['end_date','name'])
